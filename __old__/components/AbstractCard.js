...
import { moveToTrash, offerFieldChange } from '../actions';
...
const AbstractCard = ({
  ...
  <div
    className={`abstract-card flex-columns relative ${
      item.isactive ? 'active' : ''
    }`}>
    <div className="abstract-card-left-column">
      <p className="abstract-card-date">
        <span>{item.humandate}</span>
      </p>
      <h2 className="abstract-card-title">
        <span>{!minify ? item.title : item.shorten.title}</span>
      </h2>
      <div
        dangerouslySetInnerHTML={{
          __html: !minify ? item.description : item.shorten.description,
        }}
      />
    </div>
    {nostatus ? (
      ...
      <div className="abstract-card-right-column">
        {/* flag mail envoye */}
        <button
          className={`abstract-card-badge ${item.emailsent ? 'active' : ''}`}
          onClick={evt => {
            evt.preventDefault();
            evt.stopPropagation();
          ...
          <span>
            <i className="myjobboard-mail" />
          </span>
        </button>
        <span className="splitter" />
        {/* flag calendar */}
        <span
          className={`abstract-card-badge ${
            item.contact.trim() ? 'active' : ''
          }`}>
          <span>
        ...
        <span className={`abstract-card-badge ${item.event ? 'active' : ''}`}>
          <span>
            <i className="myjobboard-calendar" />
          </span>
        </span>
        <span
          className={`abstract-card-badge ${
            item.comment.trim() ? 'active' : ''
          }`}>
          <span>
